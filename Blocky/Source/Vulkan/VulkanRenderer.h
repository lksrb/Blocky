...
static void InitializeRenderingResources(game_renderer* Renderer)
{
    // Texture
    {
        // White texture for texture-less quads
        {
            ...
            u8* Pixels = stbi_load("Resources/Textures/MC/Crosshair.png", &Width, &Height, &Channels, 4);
            Assert(Channels == 4, "Failed to enforce 4 channels!");

            Renderer->DefaultWhiteTexture = TextureCreate(Renderer->Device, Renderer->PhysicalDevice, Renderer->CommandPool, Renderer->GraphicsQueue, Width, Height, Pixels, Width * Height * Channels, VK_FILTER_NEAREST);

            // Set first texture
            Renderer->TextureStack[0] = Renderer->DefaultWhiteTexture;
        }
        // GPU resources
        {
    ...
    // Quad
    {
        // Index buffer
        {
            ...
            vulkan_buffer StagingBuffer = VulkanBufferCreate(Renderer->Device, Renderer->PhysicalDevice, buffer_usage::TransferSource,
                memory_property::HostVisible | memory_property::Coherent, sizeof(u32) * c_MaxQuadIndices, true);
            ...
            Renderer->QuadIndexBuffer = VulkanBufferCreate(Renderer->Device, Renderer->PhysicalDevice, buffer_usage::TransferDestination | buffer_usage::IndexBuffer,
                memory_property::Local | memory_property::Coherent, sizeof(u32) * c_MaxQuadIndices, false);

            // Copy data to the staging buffer
            memcpy(StagingBuffer.MappedData, QuadIndices, sizeof(u32) * c_MaxQuadIndices);
...
static game_renderer GameRendererCreate(game_window Window)
{
    ...
    GameRendererResizeSwapChain(&Renderer, Window.ClientAreaWidth, Window.ClientAreaHeight);
...
static void GameRendererSubmitCube(game_renderer* Renderer, v3 Translation, v3 Rotation, v3 Scale, v4 Color, draw_layer Layer)
{
    m4 Transform = bkm::Translate(m4(1.0f), Translation)
        * bkm::ToM4(qtn(Rotation))
        * bkm::Scale(m4(1.0f), Scale);
...
static void GameRendererRender(game_renderer* Renderer, u32 ClientWidth, u32 ClientHeight)
{
    // Begin render
    {
        ...
        if (Renderer->ResizeSwapChain)
        {
            ...
            GameRendererResizeSwapChain(Renderer, Renderer->SwapChainSize.width, Renderer->SwapChainSize.height);
        }
    }
    ...
    // Record command buffer
    {
        ...
        if (Renderer->QuadIndexCount > 0)
        {
            vkCmdBindDescriptorSets(commandBuffer, VK_PIPELINE_BIND_POINT_GRAPHICS, Renderer->QuadPipeline.PipelineLayoutHandle, 0, 1, &Renderer->DescriptorSets[Renderer->CurrentFrame], 0, nullptr);
