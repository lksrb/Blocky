...
static void DX12VertexBufferSendData(dx12_vertex_buffer* VertexBuffer, ID3D12GraphicsCommandList* CommandList, void* Data, u64 DataSize)
{
    ...
    auto Barrier = DX12RendererTransition(VertexBuffer->Buffer.Resource, D3D12_RESOURCE_STATE_VERTEX_AND_CONSTANT_BUFFER, D3D12_RESOURCE_STATE_COPY_DEST);
    CommandList->ResourceBarrier(1, &Barrier);

    CommandList->CopyBufferRegion(VertexBuffer->Buffer.Resource, 0, VertexBuffer->IntermediateBuffer.Resource, 0, DataSize);

    Barrier = DX12RendererTransition(VertexBuffer->Buffer.Resource, D3D12_RESOURCE_STATE_COPY_DEST, D3D12_RESOURCE_STATE_VERTEX_AND_CONSTANT_BUFFER);
    CommandList->ResourceBarrier(1, &Barrier);
}
